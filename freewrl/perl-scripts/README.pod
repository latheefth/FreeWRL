...
=head1 Perl - VRML interface

VRML "SF" nodes (C<SFColor>, C<SFFloat>, C<SFInt32> etc) are mapped to
scalar 
perl variables with the same name.

VRML "MF" nodes (C<MFColor>, C<MFFloat>, C<MFInt32> etc) are mapped to
ref to array perl variables with the same name.

VRML C<eventOut's> are mapped to perl variables that emit a signal
when set. 

VRML C<eventIn's> are mapped to perl sub's that must be defined in the
C<url> field of the C<Script> node.
...
Lines 36-38 : The function C<initialize> that will be called at
initialization. Without it, the sphere remains white until it is
clicked upon.
