...
<!--
  <head>
   ...
   <meta name="modified"     		content="9 October 2006" />
      ...
      <xsl:if test="not(contains(local-name(ProtoBody/node()[position()=1]),'comment')) and not(contains(local-name(ProtoBody/node()[position()=2]),'comment'))">
        <xsl:call-template name="output-warning">
          <xsl:with-param name="warningString">
            ...
            <xsl:text> following first child will not be rendered.  (Authors can silence this warning by inserting a comment.)</xsl:text>
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:if>
