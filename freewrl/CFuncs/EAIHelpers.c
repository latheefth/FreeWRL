...
int mapToEAItype (indexT kwIndex) {
	switch (kwIndex) {
		...
		if (group->__protoDef) {
			...
			for (fc = 0; fc <protoDefinition_getFieldCount(myProtoDecl); fc++) {
				...
				if (retstr != NULL) {
					...
					EAI_Convert_mem_to_ASCII (0, "", mapToEAItype(protoFieldDecl_getType(thisIndex)), 
				...
				if (strlen(protoFieldDecl_getStringName(thisIndex)) == strlen(myField)) {
					if (strcmp(protoFieldDecl_getStringName(thisIndex),myField) == 0) {
						...
						*myProtoIndex = fc;
						*myType = mapToEAItype(protoFieldDecl_getType(thisIndex));
...
*/
	...
	*scripttype =0;
	...
	printf ("EAI_GetType, so we have coffset %d, ctype %c, ctmp %s\n",*fieldOffset,ctype, KEYWORDS[*accessType]);
}
