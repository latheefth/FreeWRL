...
extern char *ReWireConfigStr;
extern int haveNewReWireConfig;

void createIdleThread (void);
void checkInstalledReasonDevices (void);
void initializeInstalledReasonDevices(void);
void renderAudioCallback(signed short* interleavedBuffer, unsigned long frameCount) ;
int reasonStartAudio(void);
...
# 14 "/root/freewrl/freewrl-1.19.9/ReWire/X3DNode.h"
...
void fatalError (char *str) {
 ...
 printf ("FatalError in freewrlReWireServer: %s\n",str);
...
# 205 "FreeWRLMIDIInterface.c"
 ...
 printf ("rewire program exiting normally...\n");
...
static pthread_t configIdler;

char *totalConfigString = ((void *)0);
int totalConfigStringSize = 0;
...
void configThread (void) {
 ...
 while (1==1) {
  ...
  if (haveNewLocalMIDIConfig || haveNewReWireConfig) {
   printf ("configthread - MIDI Config changed. \n");


   sizeRequired = 100;
   if (localMidiString != ((void *)0)) sizeRequired += strlen (localMidiString);
   if (ReWireConfigStr != ((void *)0)) sizeRequired += strlen (ReWireConfigStr);


   if (sizeRequired > totalConfigStringSize) {
    totalConfigStringSize = sizeRequired;
    totalConfigString = realloc(totalConfigString,totalConfigStringSize);
   }

   totalConfigString[0] = '\n';
   totalConfigString[1] = '\0';
   if (localMidiString!=((void *)0)) strcat (totalConfigString,localMidiString);
   if (ReWireConfigStr!=((void *)0)) strcat (totalConfigString,ReWireConfigStr);
   ...
   sendMIDITableToFreeWRL(totalConfigString);

   haveNewLocalMIDIConfig = (1==0);
   haveNewReWireConfig = (1==0);
  }
...
void createConfigThread () {
 ...
 pthread_create (&configIdler, ((void *)0), (void*(*)(void *))&configThread, ((void *)0));
}
