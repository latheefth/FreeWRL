...
*********************************************************************/
# 1 "localMIDIInterface.c"
# 1 "/root/usb/FreeWRL_Reason_MIDI_Interface//"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "localMIDIInterface.c"
# 12 "localMIDIInterface.c"
...
# 2 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 5 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 11 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 21 "localMIDIInterface.c" 2

char *me="FreeWRL Reason Interface, version 1.2 Copyright CRC Canada 2007" "website http://www.crc.ca FreeWRL Website http://freewrl.sf.net " " phone 613 998 9648; email alex.stewart@crc.ca";




char *localMidiString = ((void *)0);
int localMidiStringSize = 0;


int haveNewReWireConfig = 1!=1;
int haveNewLocalMIDIConfig = 1!=1;


static pthread_t linuxReadThread;




static MIDIPortRef inPort = ((void *)0);




static MIDIPacketList outPacketList;
MIDIPacket *outPacketPtr = ((void *)0);
...
void sendMIDI (int bus,int channel, int controllerOrVelocity, int controlType, int value) {
 char midi[256];
 ...
 if (bus < 1000) {
  ...
  printf ("sendMIDI - bus %d invalid for local midi\n",bus);

 } else {

  if (outPacketPtr == ((void *)0)) {
   printf ("sendMIDI - outgoing packets not configured yet\n");
   return;
  }
  ...
  if (controlType == 1) {
   ...
   if (controllerOrVelocity < 0) {
    midi[0] = 0x80;
    midi[2] = 0;
   } else {
    midi[0] = 0x90;
    midi[2] = controllerOrVelocity;
   }
   midi[0] = midi[0] | (channel & 0x0f);
   midi[1] = value & 0xff;
...
void startUpLocalMIDI () {
 ...
 localMidiStringSize = 16384;
 localMidiString = malloc(localMidiStringSize);
 localMidiString[0] = '\n';
 ...
 for (i=0; i<10; i++) {
  ...
  localDev[i].midiSource = 1!=1;
  localDev[i].midiSink = 1!=1;
 ...
 destCount = 0;
        ...
        pthread_create (&linuxReadThread, ((void *)0), (void*(*)(void *))dataWaiting, ((void *)0));

 outPacketPtr = MIDIPacketListInit(&outPacketList);
...
# 546 "localMIDIInterface.c"
 ...
 haveNewLocalMIDIConfig = 1==1;
}


void tagDevicePresent(MIDIEndpointRef dev, int inputDirection) {
 int i;
 SInt32 uniq;
 char name[64];
 CFStringRef pname;
 ...
 localDev[i].present = 1==1;
 localDev[i].uniqueID = uniq;
 if(localDev[i].name) {free(localDev[i].name); localDev[i].name = 0;};
 localDev[i].name = strdup(name);



 if (inputDirection) {
  localDev[i].inDevice = dev;
  localDev[i].midiSource = 1==1;
  MIDIPortConnectSource(inPort, localDev[i].inDevice, (void *)i);
 } else {
  localDev[i].outDevice = dev;
  localDev[i].midiSink = 1==1;
 }
}
...
void diffFoundMidiDeviceInfo() {
 ...
 for (i=0; i<10; i++) {
  if (localDev[i].olduniqueID != localDev[i].uniqueID) {
   ...
   needToRegen = 1==1;
   if (localDev[i].olduniqueID == 0) {



   }
   if (localDev[i].uniqueID == 0) {



    localDev[i].present = 1!=1;
    if (localDev[i].midiSource)
     MIDIPortDisconnectSource(inPort, localDev[i].inDevice);
   }
...
void checkInstalledMIDIDevices() {
 int i;


 copyFoundMidiDeviceInfo();
 ...
 if (sourceCount>0) {
  ...
  for (i = 0; i < sourceCount; ++i)
   tagDevicePresent(MIDIGetSource(i),1==1);
 } else {
