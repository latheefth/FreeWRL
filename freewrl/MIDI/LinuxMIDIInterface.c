...
# 3 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 93 "/root/freewrl/freewrl-1.19.9/CFuncs/EAIheaders.h"
int createUDPSocket();
...
# 6 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 7 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 8 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 9 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
# 1 "/usr/include/strings.h" 1 3 4
# 10 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 12 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 13 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 17 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
# 1 "/usr/lib/gcc/i486-linux-gnu/4.1.2/include/stddef.h" 1 3 4
...
# 20 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2

# 1 "/root/freewrl/freewrl-1.19.9/ReWire/GeneratedHeaders.h" 1
# 22 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h" 2
...
# 45 "/root/freewrl/freewrl-1.19.9/ReWire/Eai_C.h"
...
typedef struct MDInfo MDInfo;


MDInfo midiCardInfo[64];
int midiCardInfoInitialized = 1!=1;


void initializeCFString (CFStringRef me) {
 me.len=-1; me.strptr=((void *)0);
}

void CFRelease (CFStringRef me) {
 if(me.strptr) {free(me.strptr); me.strptr = 0;}; me.len = -1;
}

void CFStringGetCString(CFStringRef pname, char *name, int i, int x) {
 strncpy(name,pname.strptr,i);
...
void zeroMIDIDeviceList() {
 ...
 if (!midiCardInfoInitialized)
  for (i=0; i<64; i++) {
   midiCardInfo[i].cardNo = -1;
   midiCardInfo[i].portNo = -1;
   midiCardInfo[i].name = ((void *)0);
   midiCardInfo[i].fileDesc = -1;
  }

 for (i=0; i<currentNumberOfMIDIDevices; i++) {
  midiCardInfo[i].cardNo = -1;
  midiCardInfo[i].portNo = -1;
  if(midiCardInfo[i].name) {free(midiCardInfo[i].name); midiCardInfo[i].name = 0;};
  midiCardInfo[i].fileDesc = -1;
 }

 currentNumberOfMIDIDevices = 0;
}

void saveMIDIDeviceInfo(int card) {
 char tempfilename[4000];
 int i,md,fd, ok;
 int jr;
 char temp[4000];
...
# 129 "LinuxMIDIInterface.c"
 ...
 fd = open(tempfilename,00);
 if (fd >=0) {
  jr = read (fd,temp,4000);
  ...
  if ((jr > 1) && (jr < 4000)) {
   ...
   strcat (temp,"(card name out of range)");
  }

  close(fd);
 } else {
  printf ("saveMIDIDeviceInfo: could not open ID file (%s), skipping...\n",tempfilename);
  return;
 }
 ...
 do {
  ...
  sprintf (tempfilename,"/dev/snd/midiC%dD%d",card,i); fd = open(tempfilename,02);
  ...
  if (fd >= 0) {
   ...
   midiCardInfo[currentNumberOfMIDIDevices].cardNo = card;
   midiCardInfo[currentNumberOfMIDIDevices].portNo = i;
   midiCardInfo[currentNumberOfMIDIDevices].fileDesc = fd;

   currentNumberOfMIDIDevices ++;
  } else {
 ...
 for (i=whereToStartmidiCardInfo; i<currentNumberOfMIDIDevices; i++) {

  if (currentNumberOfMIDIDevices == (whereToStartmidiCardInfo+1)) {
   midiCardInfo[i].name = strdup(temp);
  } else {

   sprintf (tempfilename, "%s_%c",temp,(char)('A'+i-whereToStartmidiCardInfo));
   midiCardInfo[i].name = strdup(tempfilename);
  }
...
# 219 "LinuxMIDIInterface.c"
 ...
 while (*ptr != '\0') {
  ...
  goodOne = !strncmp ("USB-Audio - ",ptr,strlen("USB-Audio - "));
...
int MIDISend (int bus, Byte *data, int len) {
 ...
 outDevice = (bus-1000);
 outChannel = outDevice & 0x0f;
 outDevice = (outDevice & 0xfff0) >> 4;
 ...
 if ((outDevice >= 64) || (outDevice < 0)) {
  printf ("MIDISend: outDevice > devices... skipping packet\n");
  return 1!=1;
 }


 if (midiCardInfo[outDevice].fileDesc < 0) {
...
# 312 "LinuxMIDIInterface.c"
 ...
 if (x != len) {
  printf("MIDISend - just wrote %d bytes, expected to wrinte %d\n",x,len);
  return 1!=1;
 }
...
MIDIPacket * MIDIPacketNext(MIDIPacket *inPacket) {
 printf ("MIDIPacketNext called - does nothing\n");
 return ((void *)0);
}
...
int MIDIGetNumberOfSources (void) {
 ...
 fd = open("/proc/asound/cards",00, 0);

 if (fd < 0) {
  perror ("FreeWRL ReWireServer");
  return 0;
 }


 while (read(fd,fline,4000)>0) {
  if ((listingOfMIDIDevices == ((void *)0)) || (strcmp(fline,listingOfMIDIDevices) != 0)) {




   if(listingOfMIDIDevices) {free(listingOfMIDIDevices); listingOfMIDIDevices = 0;};
   listingOfMIDIDevices = strdup(fline);
   return (newMIDI());
  }
 }
 return currentNumberOfMIDIDevices;
}
...
# 397 "LinuxMIDIInterface.c"
 if (midiCardInfo[*dev].fileDesc == -1) {
  ...
  if (midiCardInfo[*dev].fileDesc <= 0) {
   printf ("MIDIPortConnectSource - difficulty with opening port %d card %d (%s)\n",
    midiCardInfo[*dev].portNo,midiCardInfo[*dev].cardNo,midiCardInfo[*dev].name);
  }
 }
}
...
# 421 "LinuxMIDIInterface.c"
 if (midiCardInfo[*dev].fileDesc != -1) {
  ...
  close(midiCardInfo[*dev].fileDesc);
 }
}
...
void MIDIObjectGetIntegerProperty(MIDIEndpointRef dev, int i, int *ret) {
 switch (i) {
  ...
  case 114:
  case 115:
    printf ("MIDIObjectGetIntegerProperty called but not decoded yet; device %d, kMIDIProp... %d\n",*dev,i);
     break;
  case 116:
   if ((*dev>=0) && (*dev<currentNumberOfMIDIDevices)) *ret = midiCardInfo[*dev].cardNo;
   else *ret = -1;
   break;
  case 117: break;
  default: *ret=0;
...
void MIDIObjectGetStringProperty(MIDIEndpointRef dev, int i, CFStringRef *ret) {
 ...
 switch (i) {
  ...
  case 116:
  default:
   printf ("MIDIObjectGetStringProperty called but not decoded yet; device %d, kMIDIProp... %d\n",*dev,i);
   break;
  case 117:
  case 102:
   if ((*dev>=0) && (*dev<currentNumberOfMIDIDevices)) {
    ret->strptr = strdup(midiCardInfo[*dev].name);
    ret->len = strlen(ret->strptr);
   }
...
MIDIPacket *MIDIPacketListInit(MIDIPacketList *pl) {
 ...
 retval = (MIDIPacket *)malloc(sizeof (MIDIPacketList));
 retval->timeStamp = 0;
 retval->length = 0;
 ...
 while (1==1) {
  ...
  for (i=0; i<currentNumberOfMIDIDevices; i++) {
   if (midiCardInfo[i].cardNo!=-1) {
    curFD = midiCardInfo[i].fileDesc;
    __asm__ __volatile__ ("btsl %1,%0" : "=m" (((&rfds2)->__fds_bits)[((curFD) / (8 * sizeof (__fd_mask)))]) : "r" (((int) (curFD)) % (8 * sizeof (__fd_mask))) : "cc","memory");
    if (curFD > max) max = curFD;

   }
  }
  retval = select(max+1, &rfds2, ((void *)0), ((void *)0), &tv2);
  ...
  if (retval > 0) {
   for (i=0; i<currentNumberOfMIDIDevices; i++) {
    if (midiCardInfo[i].cardNo!=-1) {
     if ((__extension__ ({register char __result; __asm__ __volatile__ ("btl %1,%2 ; setcb %b0" : "=q" (__result) : "r" (((int) (midiCardInfo[i].fileDesc)) % (8 * sizeof (__fd_mask))), "m" (((&rfds2)->__fds_bits)[((midiCardInfo[i].fileDesc) / (8 * sizeof (__fd_mask)))]) : "cc"); __result; }))) {
