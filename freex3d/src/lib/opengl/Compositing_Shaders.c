...
\n\
  ...
  vertexcolor = clamp(vec4(vec3(ambient+diffuse+specular),myAlph), 0.0, 1.0);\n\
	...
	const GLchar **fragmentSource, unsigned int whichOne, int usePhongShading) 
{
	...
	if(DESIRE(whichOne,MATERIAL_APPEARANCE_SHADER) || DESIRE(whichOne,TWO_MATERIAL_APPEARANCE_SHADER)){
		...
		Plug(SHADERPART_VERTEX,plug_vertex_lighting_ADSLLightModel,CompleteCode,&unique_int); //use lights
	}
	//lines and points 
