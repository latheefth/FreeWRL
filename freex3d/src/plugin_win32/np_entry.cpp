...
///This functions maps the plugin functions to the NPPluginFunc struct (methods table) and handles it
///to the browser. It is used only in MicroSoft S.O.
...
/// Returns the mimetype description for the plugin that has been defined in the dll version (windows) and/or
/// it is returned by a global function in the plugin
char *
...
/// This function initializes the plugin passing to the NPNetscapeFuncs struct the function table passed
/// from the navigator to the plugin. This function supports a second argument if compiled for UNIX/Linux.
/// The second argument is the NPPluginFuncs struct that Windows passes through NP_GetEntryPoint.
NPError OSCALL
...
/// Needed destruction operation go here.
